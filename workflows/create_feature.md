...
- **Upstream**:
  ...
  - **Research**: `RESEARCH-XXX` (if research was done first)
- **Downstream**:
  - **Plan**: `PLAN-XXX` (implementation plan)
...
3. **Link to Kanban Task** (if applicable):
   ...
   ```bash
   python3 dev_ops/scripts/kanban_ops.py link TASK-XXX FEAT-XXX --relation output
   ```
...
- [ ] Feature doc created in `dev_ops/features/`
