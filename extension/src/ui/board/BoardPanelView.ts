  ...
  infoBtn.addEventListener('mouseenter', (e) => {
    ...
    showTaskDetailsPopover(task, rect.left, rect.top, () => {
      ...
      hideTimeout = setTimeout(() => {
        const container = document.getElementById('details-popover-container');
...
// --- Popover Logic ---
function showTaskDetailsPopover(task, x, y, onEnter, onLeave) {
  // Remove existing
  ...
  // Allow pointer events to pass through wrapper, but catch on popover
  ...
  popover.style.pointerEvents = 'auto'; // Re-enable pointer events

  // Position: Above the button
  popover.style.position = 'absolute';
  popover.style.left = Math.max(10, Math.min(x - 220, window.innerWidth - 275)) + 'px';
  popover.style.top = (y - 145) + 'px';

  // Handle hover
  if (onEnter) popover.addEventListener('mouseenter', onEnter);
  if (onLeave) popover.addEventListener('mouseleave', onLeave);
  ...
  const closeHandler = (e) => {
    // If clicking outside popover
    if (e.type === 'click' && !popover.contains(e.target)) {
      container.remove();
      cleanup();
    }
    if (e.key === 'Escape') {
