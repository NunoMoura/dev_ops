...
export class KanbanTreeProvider implements vscode.TreeDataProvider<KanbanNode> {
  ...
  private readonly onDidChangeEmitter = new vscode.EventEmitter<KanbanNode | undefined>();
  ...
  getParent(element: KanbanNode): KanbanNode | undefined {
  ...
  async revealTask(taskId: string | undefined, view: vscode.TreeView<KanbanNode>): Promise<void> {
    if (!taskId) {
      return;
    }
    await this.ensureLoaded();
...
export class KanbanManagerProvider implements vscode.TreeDataProvider<KanbanManagerNode> {
  ...
  constructor(private readonly treeProvider: KanbanTreeProvider) {
  ...
  async getChildren(element?: KanbanManagerNode): Promise<KanbanManagerNode[]> {
    if (element) {
      return [];
    }
    if (!this.board) {
  ...
  getTreeItem(element: KanbanManagerNode): vscode.TreeItem {
  ...
  getParent(_element: KanbanManagerNode): undefined {
    return undefined;
  }
...
class KanbanManagerDragAndDropController implements vscode.TreeDragAndDropController<KanbanManagerNode> {
  ...
  async handleDrop(target: KanbanManagerNode | undefined, dataTransfer: vscode.DataTransfer): Promise<void> {
