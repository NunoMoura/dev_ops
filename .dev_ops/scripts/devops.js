...
var require_path = __commonJS({
  "node_modules/.pnpm/fast-glob@3.3.3/node_modules/fast-glob/out/utils/path.js"(exports2) {
    ...
    function makeAbsolute(cwd2, filepath) {
      return path5.resolve(cwd2, filepath);
    }
...
// src/services/setup/projectAuditService.ts
var path = __toESM(require("path"));
var ProjectAuditService = class {
  ...
  async checkTriggers(triggers, contentSearch) {
      ...
      if (t.includes("*")) {
        ...
        if (await this.workspace.exists(p)) {
          if (contentSearch) {
            ...
            if (content.includes(contentSearch.toLowerCase())) {
              return true;
            }
          } else {
            return true;
...
// src/services/setup/configService.ts
...
var ConfigService = class {
  ...
  getConfigPath() {
    return path2.join(this.workspace.root, ".dev_ops", "config.json");
  }
  async readConfig() {
...
var CoreTaskService = class {
  ...
  async getBoardPath() {
    return path3.join(this.workspace.root, ".dev_ops", "board.json");
  }
  async readBoard() {
    ...
    const tasksDir = path3.join(path3.dirname(boardPath), "tasks");
    const items = [];
  ...
  async writeBoard(board) {
    const boardPath = await this.getBoardPath();
    const dir = path3.dirname(boardPath);
    if (!await this.workspace.exists(dir)) {
      await this.workspace.mkdir(dir);
    }
    const persistenceObject = {
      ...
      items: []
      // Items are stored in tasks/*.json
    };
    await this.workspace.writeFile(boardPath, JSON.stringify(persistenceObject, null, 2));
  }
  async saveTask(task) {
    const boardPath = await this.getBoardPath();
    const devOpsDir = path3.dirname(boardPath);
    const tasksDir = path3.join(devOpsDir, "tasks");
    if (!await this.workspace.exists(tasksDir)) {
      await this.workspace.mkdir(tasksDir);
    }
    const taskPath = path3.join(tasksDir, `${task.id}.json`);
  ...
  async claimTask(taskId, driver, ownerOverride) {
    const board = await this.readBoard();
    const task = board.items.find((t) => t.id === taskId);
    if (!task) {
      throw new Error(`Task ${taskId} not found`);
    ...
    try {
      ...
      const ctxDir = path3.join(this.workspace.root, ".dev_ops", "context");
      if (!await this.workspace.exists(ctxDir)) {
        await this.workspace.mkdir(ctxDir);
      }
      const contextPath = path3.join(ctxDir, `${taskId}.md`);
      let currentContext = "";
...
`;
  ...
  async getCurrentTask() {
    const taskPath = path3.join(this.workspace.root, ".dev_ops", ".current_task");
    const activeTaskPath = path3.join(this.workspace.root, ".dev_ops", "active_task");
...
// src/services/analysis/scopeService.ts
var path4 = __toESM(require("path"));
var CoreScopeService = class {
  constructor(workspace2, root) {
    this.workspace = workspace2;
    this.root = root;
  }
  /**
  ...
  async getScope(targetPath) {
    ...
    const globPattern = path4.join(specDir, "**/*");
    const files = await this.workspace.findFiles(globPattern, "**/node_modules/**");
    return {
      component: path4.basename(specDir),
      description,
      specPath: path4.relative(this.root, specPath),
      dependencies,
      files: files.map((f) => path4.relative(this.root, f))
    };
  }
...
// src/infrastructure/nodeWorkspace.ts
var fs = __toESM(require("fs"));
var import_fast_glob = __toESM(require_out4());
var NodeWorkspace = class {
  constructor(root) {
    this.root = root;
  }
  ...
  async writeFile(path5, content) {
    await fs.promises.writeFile(path5, content, "utf8");
  }
  async exists(path5) {
...
var cwd = process.cwd();
var workspace = new NodeWorkspace(cwd);
var taskService = new CoreTaskService(workspace);
program2.name("devops").description("DevOps CLI for agent-driven workflows").version("0.0.1");
program2.command("detect").description("Detect project stack, docs, and tests (JSON output)").option("--scope <scope>", "Detection scope (architecture, stack, docs, tests)").action(async (options) => {
...
program2.command("claim-task").description("Claim a task and move it to In Progress (or specified column)").option("--id <id>", "Task ID (e.g. TASK-123)").option("--column <column>", "Target column ID", "col-in-progress").action(async (options) => {
  if (options.column) {
