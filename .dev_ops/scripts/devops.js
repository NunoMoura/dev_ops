...
var CoreTaskService = class {
  ...
  async saveTask(task) {
    ...
    const bundleDir = path3.join(tasksDir, task.id);
    if (!await this.workspace.exists(bundleDir)) {
      await this.workspace.mkdir(bundleDir);
    }
    const taskPath = path3.join(bundleDir, "task.json");
  ...
  async claimTask(taskId, driver, ownerOverride) {
    ...
    try {
      ...
      const tasksDir = path3.join(this.workspace.root, ".dev_ops", "tasks");
      const bundleDir = path3.join(tasksDir, taskId);
      if (!await this.workspace.exists(bundleDir)) {
        await this.workspace.mkdir(bundleDir);
      }
